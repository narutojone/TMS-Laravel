    ...
    <div class="col-lg-12">
        <div class="wrapper wrapper-content animated fadeInUp" style="padding-bottom: 0;">
            <div class="alert alert-danger m-b-none">
                <strong>Advarsel!</strong> Oppgaven du nå skal opprette, er kun ment til å lage huskelapper for særegne oppgaver knyttet til kunden. Oppgaver som gjelder kontrakten, som f.eks bokføring, lønn og andre standariserte tjenester skal IKKE lages via denne funksjonen. Disse må du sende til kontraktoppdatering, slik vi til enhver til holder kontroll på hvilke tjenester kunden har hos oss. Merk at oppgavene du lager vil bli fulgt opp på lik linje med alle andre oppgaver.
            </div>
        </div>
    </div>
</div>
        ...
        <div class="wrapper wrapper-content">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form id="form" class="form-horizontal" role="form" method="POST" action="{{ action('TaskController@storeCustom', $client) }}">
                        {{ csrf_field() }}
                                ...
                                <select class="form-control chosen-select" name="user_id" id="user_id">
                                    @if(Auth::user()->hasRole(\App\Repositories\User\User::ROLE_ADMIN))
                                        @foreach (App\Repositories\User\User::active()->orderBy('name')->get() as $user)
                                            <option value="{{ $user->id }}" {{ (old('user_id') == $user->id) ? ' selected' : (($user->id == auth()->user()->id) ? ' selected' : '') }}>{{ $user->name }}</option>
                                        @endforeach
                                    @else
                        ...
                        <div class="form-group{{ $errors->has('repeating') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-10">
                                ...
                                <input type="checkbox" class="js-switch" value="1" id="repeating" name="repeating"{{ old('repeating') ? ' checked' : '' }}>
